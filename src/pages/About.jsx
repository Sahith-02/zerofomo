    ...
    const observerCallback = (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          entry.target.classList.add("aboutpage-section-visible");
        }
      });
    };
    ...
    <div>
      ...
      <div className="aboutpage-main-container">
        {sections.map((section, index) => (
          <div
            key={section.id}
            className={`aboutpage-content-section ${
              section.id === 4 ? "aboutpage-section-special" : ""
            }`}
            ref={(el) => (sectionsRef.current[index] = el)}
          >
            {section.id === 4 ? (
              <div className="aboutpage-section-content aboutpage-special-layout">
                <h2 className="aboutpage-section-title">{section.title}</h2>

                {section.image && (
                  <div className="aboutpage-image-container aboutpage-slide4-image">
                    <img
                      src={section.imageUrl}
                      alt={section.altText}
                      className="aboutpage-section-image aboutpage-slide4-image"
                    />
                  </div>
                )}

                <div
                  className="aboutpage-section-text"
              ...
              <div
                className={`aboutpage-section-content aboutpage-layout-${section.imagePosition}`}
              >
                {/* Desktop view maintains original layout */}
                <div className="aboutpage-desktop-view">
                  {section.id === 2 ? (
                    <>
                      <div className="aboutpage-image-container">
                        <img
                          src={section.imageUrl}
                          alt={section.altText}
                          className="aboutpage-section-image"
                        />
                      </div>

                      <div className="aboutpage-text-content">
                        <h2 className="aboutpage-section-title">
                          {section.title}
                        </h2>
                        <div className="aboutpage-section-text">
                          {section.content.split("\n").map((paragraph, i) => (
                            <p key={i}>{paragraph.trim()}</p>
                          ))}
                        </div>
                      ...
                      {section.image && section.imagePosition === "left" && (
                        <div className="aboutpage-image-container">
                          {section.imageContent && (
                            <div className="aboutpage-scrabble-tiles">
                              {section.imageContent}
                            </div>
                          )}
                          <img
                ...
                <div className="aboutpage-mobile-view">
                  <h2 className="aboutpage-section-title">{section.title}</h2>
